DROP TABLE IF EXISTS `financial_report_sum`;
CREATE VIEW `financial_report_sum` AS select 
cin.*,
d.nip,
d.name,
f.taxi_number,
...
(sum(if( cf.financial_type_id = 11, cf.amount, 0)) - sum(if( cf.financial_type_id = 6, cf.amount, 0))) as selisi_ks
...
group by cin.pool_id, f.fleet_model_id, cin.shift_id,  cin.operasi_time ;


-- Dumping structure for view db2.financial_report_sum_all
-- Menghapus tabel sementara dan menciptakan struktur VIEW terakhir
DROP TABLE IF EXISTS `financial_report_sum_all`;
CREATE VIEW `financial_report_sum_all` AS select 
cin.*,
d.nip,
d.name,
f.taxi_number,
...
join drivers d on (cin.driver_id = d.id) 
join fleets f on (cin.fleet_id = f.id)
group by cin.pool_id, f.fleet_model_id, cin.operasi_time ;
