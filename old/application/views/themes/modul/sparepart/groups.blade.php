...
@section('content')
	<div class="block">
        ...
        <div class="block-body collapse in" id="widgetGroup"><br>
         	{{ Form::open('sparepart/groups','POST',array('class'=>'form-horizontal')) }}
        ...
        <div class="block-body collapse in" id="widgetGroup1">
         	<table class="table table-condensed">
	            ...
	            @foreach($groups->results as $group)
	            	<tr>
	            		<td class="span1">{{ $group->id }}</td>
	            		<td>{{ $group->sp_group }}</td>
	            		<td class="span2">
                    <a class="edit_toggler btn btn-success btn-mini" rel="{{ $group->id }}" data-sp="{{ $group->sp_group }}" ><i class="icon-edit icon-white"></i></a> 
                    <a class="delete_toggler btn btn-danger btn-mini" rel="{{ $group->id }}"><i class="icon-remove-sign icon-white"></i></a>
                  </td>
	            	</tr>
	            @endforeach
	            </tbody>
  ...
  <div class="modal hide fade" id="delete_group">
      ...
      <div class="modal-footer">
        {{ Form::open('sparepart/delgroup', 'POST') }}
